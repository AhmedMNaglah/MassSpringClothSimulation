	...
	if (m_SimRunning)
	{
		while(m_LastTime < Time)
		{
			...
			if(DeltaTime > m_MaxTimeStep)
			{
	 		...
	 		m_PhysEnv.Simulate(DeltaTime,m_SimRunning, m_HuenIterations, ErrorAnalysis);
...
void COGLView::OnSetTimeProperties() 
{
	...
	dialog.m_HuenIterations = m_HuenIterations;
	dialog.ErrorAnalysis = ErrorAnalysis;
	if (dialog.DoModal())
	{
		m_TimeIterations = dialog.m_Iterations;
		m_UseFixedTimeStep = dialog.m_FixedTimeSteps;
		m_MaxTimeStep = dialog.m_MaxTimeStep;
		m_HuenIterations = dialog.m_HuenIterations;
		ErrorAnalysis = dialog.ErrorAnalysis;
	}
}

...
///////////////////////////////////////////////////////////////////////////////
void CPhysEnv::EulerIntegrate( float DeltaTime, bool ErrorAnalys)
{
...
///////////////////////////////////////////////////////////////////////////////
void CPhysEnv::MidPointIntegrate( float DeltaTime, bool ErrorAnalys)
{
/// Local Variables ///////////////////////////////////////////////////////////
	float		halfDeltaT;
	float       QrtT;
///////////////////////////////////////////////////////////////////////////////
	...
	IntegrateSysOverTime(m_CurrentSys,m_TempSys[0],m_TargetSys,DeltaTime);
		...
		if (ErrorAnalys)
	{
	// Error Estimation 1'st Step
	halfDeltaT = DeltaTime / 2.0f;
	QrtT = DeltaTime / 4.0f;
	// TAKE A HALF STEP AND UPDATE VELOCITY AND POSITION - Find y at half-interval
	IntegrateSysOverTime(m_CurrentSys,m_CurrentSys,m_TempSys[1],QrtT);
	...
	// Error Estimation 2nd  Step
	// TAKE A HALF STEP AND UPDATE VELOCITY AND POSITION - Find y at half-interval
	IntegrateSysOverTime(m_TempSys[2],m_TempSys[2],m_TempSys[3],QrtT);
...
///////////////////////////////////////////////////////////////////////////////
void CPhysEnv::HeunIntegrate( float DeltaTime, int HuenIter,  bool ErrorAnalys)
{
	// 
	/// Local Variables ///////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
		for (int i=0; i<HuenIter; i++)
	{
		if (i==0)
		m_TempSys[0] = m_CurrentSys;
		else 
		m_TempSys[0] = m_TempSys[1]; // Set the Source System

		// Take 1st Iteration & Store in m_TempSys[0] - Compute velocities & positions at End of Interval
		IntegrateSysOverTime(m_CurrentSys,m_TempSys[0],m_TempSys[1],DeltaTime);

		// COMPUTE FORCES USING THESE NEW POSITIONS AND VELOCITIES - Compute slope at End of Interval
		ComputeForces(m_TempSys[1]);

		// Compute Mean Velocities & Forces to Perform the 2nd Iteration 
		m_TempSys[1] -> v.x = ( m_TempSys[1] -> v.x + m_TempSys[0] ->v.x ) /2.0f ;
		m_TempSys[1] -> v.y = ( m_TempSys[1] -> v.y + m_TempSys[0] ->v.y ) /2.0f ;
		m_TempSys[1] -> v.z = ( m_TempSys[1] -> v.z + m_TempSys[0] ->v.z ) /2.0f ;

		m_TempSys[1]-> f.x = ( m_TempSys[1] -> f.x + m_TempSys[0] ->f.x ) /2.0f ;
		m_TempSys[1]-> f.y = ( m_TempSys[1] -> f.y + m_TempSys[0] ->f.y ) /2.0f ;
		m_TempSys[1] -> f.z= ( m_TempSys[1] -> f.z + m_TempSys[0] ->f.z ) /2.0f ;
	}    
...
void CPhysEnv::Simulate(float DeltaTime, BOOL running, int HuenIterations, BOOL ErrorAnalysis )
{
		...
		if (running)
		{
			...
			else
			{
				...
				case EULER_INTEGRATOR:
					EulerIntegrate(TargetTime-CurrentTime, ErrorAnalysis);
					break;
				case MIDPOINT_INTEGRATOR:
					MidPointIntegrate(TargetTime-CurrentTime , ErrorAnalysis);
					break;
				case HEUN_INTEGRATOR:
					HeunIntegrate(TargetTime-CurrentTime, HuenIterations, ErrorAnalysis);
					break;
				case RK4_INTEGRATOR:
